function Validator(form, childForm, min, str) {
    ...
    for (let childFormEl of childFormEls) {
        //appendChild them 1 node vao vi tri cuoi
        //setAttribute them 1 class="error-message"
    ...
    function ruleChecking(inpEl, str) {
        ...
        if (inpAttr === 'email') {
            ...
            return regex.test(inpEl.value) ? undefined : messageAdding(inpEl, str || `Vui lòng nhập tối thiểu 7 đến 15 kí tự gồm 1 chữ số và 1 ký tự đặc biệt`, str || 'red');
    ...
    for (let inpEl of inpEls) {
        ...
        //onblur is active when user move pointer outside of input
        inpEl.onblur = () => {
            if (inpEl.value) {
                ruleChecking(inpEl);
            } else {
